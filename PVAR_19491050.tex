...
for the latter is to ascertain whether some regions drive the results
...
and to include these regional subsamples. Additionally, as another way
of summarising and visualizing the VAR results, it might be useful to
include forecast error variance decompositions (FEVDs). This method
determines how much variation between the endogenous variables is due to
which shock. The subsequent section endeavors to attempt the three
...
models are estimated using GMM, and a Cholesky decomposition is used to
identify the structural error terms. Impulse response functions are
generated accordingly. Unlike Atems \& Jones
(\protect\hyperlink{ref-atems}{2015}), however, our results are
...
To test for robustness given new data, we estimate two baseline PVARs.
The first includes only the sample of the annual series from 1930 to
2005. It is thus an exact replication of Atems \& Jones
(\protect\hyperlink{ref-atems}{2015}). The second model applies the PVAR
methodology to the full dataset, including new data. The intuition is
that the model by Atems \& Jones (\protect\hyperlink{ref-atems}{2015})
is robust to new data if the two models display similar results.
...
COIRF full sample:\footnote{All confidence intervals are calculated at
  ...
  100 draws. The low number of draws were chosen due computational
  limitations.}
...
\begin{figure}[H]
\includegraphics[width=1\linewidth]{images/Fig5_fevd_full} \caption{\label{Fig5}}\label{fig:Fig5}
\end{figure}
...
\begin{center}\includegraphics[width=0.49\linewidth,height=0.35\textheight]{images/Income_Levels_Intercept} \includegraphics[width=0.49\linewidth,height=0.35\textheight]{images/Income_Levels_Trend} \end{center}

\begin{center}\includegraphics[width=0.49\linewidth,height=0.35\textheight]{images/Gini_Growth_Intercept} \includegraphics[width=0.49\linewidth,height=0.35\textheight]{images/Gini_Growth_Trend} \end{center}

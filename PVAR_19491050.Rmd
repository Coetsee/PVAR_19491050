...
The baseline PVAR estimated in this section will follow the same methodology described in @atems, using equation \ref{eq2}. The models are estimated using GMM, and a Cholesky decomposition is used to identify the structural error terms. Impulse response functions are generated accordingly. Unlike @atems, however, our results are displayed using orthogonalized IRFs.^[This choice is primarily due to our usage of a new statistical package in R, 'panelvar', developed recently by @sigmund2019panel.]

To test for robustness given new data, we estimate two baseline PVARs. The first includes only the sample of the annual series from 1930 to 2005. It is thus an exact replication of @atems. The second model applies the PVAR methodology to the full dataset, including new data. The intuition is that the model by @atems is robust to new data if the two models display similar results.


OIRF for new data:

```{r Fig3,fig.cap="\\label{Fig2}", echo = FALSE, message = FALSE, warning = FALSE, out.width="100%", fig.height = 5, fig.pos="H"}
